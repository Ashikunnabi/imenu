...
class Product {
    ...
    **/
    ...
    set_available_category = () => {
        let self = this;
        $.ajax({
            url: available_category_list_api_url,
            type: "GET",
            success: function (resp) {
                $.map(resp.data, function (value, index) {
                    let class_active_tab = (index === 0) ? 'active': '';
                    let class_active_tab_pane = (index === 0) ? 'in active': '';
                    $('.aa-product-inner .aa-products-tab').append(
                        `<li class="${class_active_tab}">
                            <a href="#${value.html_id}" data-toggle="tab">${value.name}</a>
                        </li>`
                    );
                    $('.aa-product-inner .tab-content').append(
                        `<div class="tab-pane fade ${class_active_tab_pane}" id="${value.html_id}">
                            <ul class="aa-product-catg"></ul>
                        </div>`
                    );

                    // set products at category
                    self.product(value);
                });
            },
    ...
    product_template = (product) => {
        ...
        <li>
            <figure>
                ...
                <a class="aa-add-card-btn cart_btn_${product.hashed_id}" href="#" data-id="${product.hashed_id}" data-quantity="1">
    ...
    depricated_product_template = (product) => {
        let image_url = product.image_url ? product.image_url : '/static/base/img/no_image.png';
        return `            
            <li>
                <figure>
                    <a class="aa-product-img" href="product-details/${product.hashed_id}/">
...
<!--                        <span class="aa-product-price">$${product.jobbar_price}</span>-->
<!--                        <span class="aa-product-price"><del>$${product.previous_price}</del></span>-->
                    </figcaption>
                </figure>
                <div class="aa-product-hvr-content">
                    <a
...
<!--                                                </select>-->
                                        ...
                                        <div class="aa-prod-view-bottom" id="special_order_div_${product.hashed_id}" style="display: none">
                                            <img src="/static/base/company/img/special_order.png" alt="product_image" style="width: 100%">
                                        </div>
                                        <div class="aa-prod-view-bottom unauthorized_order" id="unauthorized_order_div_${product.hashed_id}">
                                            <p class="text-danger">* You are not authorized to purchase this product due to vendor restrictions.</p>
                                            <p><a href="${product.authorization_form_url}">Click here!</a> to apply for authorization.</p>
    ...
    product = (available_category_object, length=12) => {
        let self = this;
        $.ajax({
            url: products_based_on_category_api_url + `?category_hashed_id=${available_category_object.hashed_id}&length=${length}`,
            type: "GET",
            success: function (resp) {
                $.map(resp.data, function (value, index) {
                    $(`#${available_category_object.html_id} ul`).append(
