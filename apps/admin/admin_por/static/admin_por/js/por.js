...
class POR {
                ...
                <tr style="background:${(product.pending == 0) ? '#d3ffd3' : '#ffdada'}">
                    ...
                    ${(product.pending == 0) ? `<td>
                        <span 
                            ...
                            title="Pending quantity for receive"
    ...
    **/
    fetch_products = () => {
        let self = this
        let purchase_order_uuid = $("#purchase_order").val()

        $.ajax({
            url: purchase_order_api_url + `${purchase_order_uuid}/`,
            type: "GET",
            success: function (resp) {
                self.response = resp
    ...
    **/
    call_products = () => {
        ...
        if (purchase_order_uuid === "Search purchase order") {
            notify("Select purchase order", 'error', 5000);
            return
        }
        ...
        $(document).on('click', '#update_stock_submit_button', function (e) {
            ...
            $.each(self.response.data.products, function (i, v) {
                ...
                if (parseInt($(`#cart_${v.uuid}`).val()) > parseInt($(`#cart_${v.uuid}`).attr("max"))) {
                    notify('Purchase quantity can not be more than pending quantity', 'error', 5000);
                    return
                }
                ...
                let data = {
                    ...
                    unit_price: v.price,
                    current_received: parseInt($(`#cart_${v.uuid}`).val()),
